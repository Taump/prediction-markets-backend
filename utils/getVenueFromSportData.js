...
exports.getVenueFromSportData = async (feedName, timestamp) => {
    ...
    if (!competitionId) {
        console.error('log(venue): sport data service don \'t support this competition', feedName);
        return null;
    }
    ...
    const fixture = fixtures.find(({ fixture, teams }) => {
        ...
        return tsEqual && l(homeTeam1, homeTeam2) >= (homeMinLength / 2) && l(awayTeam1, awayTeam2) >= (awayMinLength / 2);
    });

    if (!fixture) {
        console.error('log(venue): fixture not found', feedName);
        return null;
    };
